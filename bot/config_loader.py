...
class AppConfig:
    ...
    tech_support_cisco_open_id: str
    tech_support_cisco_name: str
    tech_support_assignees: list
    tech_support_state_file: str
    tech_support_poll_enabled: bool
    tech_support_poll_seconds: int
    tech_support_event_path: str
    tech_support_lark_app_id: str
    tech_support_lark_app_secret: str
...
def load_config() -> AppConfig:
    ...
    return AppConfig(
        ...
        tech_support_cisco_open_id=str((workflow.get("tech_support") or {}).get("cisco_open_id") or "").strip(),
        tech_support_cisco_name=str((workflow.get("tech_support") or {}).get("cisco_name") or "Cisco-BE").strip(),
        tech_support_assignees=[
            {
                "name": str(x.get("name") or "").strip(),
                "open_id": str(x.get("open_id") or "").strip(),
            }
            for x in ((workflow.get("tech_support") or {}).get("assignees") or [])
            if isinstance(x, dict) and str(x.get("open_id") or "").strip()
        ] or (
            [
                {
                    "name": str((workflow.get("tech_support") or {}).get("cisco_name") or "Cisco-BE").strip(),
                    "open_id": str((workflow.get("tech_support") or {}).get("cisco_open_id") or "").strip(),
                }
            ]
            if str((workflow.get("tech_support") or {}).get("cisco_open_id") or "").strip()
            else []
        ),
        tech_support_state_file=str(
            (workflow.get("tech_support") or {}).get(
                "state_file",
                "/opt/botchain-shared/tech_support_tickets.json",
            )
        ),
        tech_support_poll_enabled=bool((workflow.get("tech_support") or {}).get("poll_enabled", True)),
        tech_support_poll_seconds=int((workflow.get("tech_support") or {}).get("poll_seconds", 45) or 45),
        tech_support_event_path=str(
            (workflow.get("tech_support") or {}).get("event_path") or "/workflow/tech-support/event"
        ).strip(),
        tech_support_lark_app_id=str((workflow.get("tech_support") or {}).get("lark_app_id") or "").strip(),
        tech_support_lark_app_secret=str((workflow.get("tech_support") or {}).get("lark_app_secret") or "").strip(),
        tech_support_auto_learn=bool((workflow.get("tech_support") or {}).get("auto_learn", True)),
        tech_support_auto_learn_min_chars=int((workflow.get("tech_support") or {}).get("auto_learn_min_chars", 20) or 20),
        tech_support_auto_learn_state_file=str(
            (workflow.get("tech_support") or {}).get(
